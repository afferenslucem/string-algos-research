import {
    getSubstringBMBadCharacter as getSubstringBMBadCharacterMap,
} from '../algorithms/bm/bad-symbol/map/bm-bad-character-search';
import { getSubstringBMGoodSuffix } from '../algorithms/bm/good-suffix/bm-good-suffix-search';
import { Suite } from 'benchmark';
import {
    getSubstringBMBadCharacter as getSubstringBMBadCharacterArray,
} from '../algorithms/bm/bad-symbol/array/bm-bad-character-search';

const alphabet = ' !\'(),-.0123456789:;?ABCDEFGJLMNOQRSVabcdefghijklmnopqrstuvxyz«»àèéêîôû́АБВГДЕЗИКМНОПРСТУФЧШЭЯабвгдежзийклмнопрстуфхцчшщъыьэюя—';

const length = 10520;

export function bmBadSymbolsCompare(): void {
    const text = `— Eh bien, mon prince. Gênes et Lucques ne sont plus que des apanages, des поместья, de la famille Buonaparte. Non, je vous préviens que si vous ne me dites pas que nous avons la guerre, si vous vous permettez encore de pallier toutes les infamies, toutes les atrocités de cet Antichrist (ma parole, j'y crois) — je ne vous connais plus, vous n'êtes plus mon ami, vous n'êtes plus мой верный раб, comme vous dites 1. Ну, здравствуйте, здравствуйте. Je vois que je vous fais peur 2, садитесь и рассказывайте.
    ...
    [
        ...
        `recommandé à l'impératrice-mère par sa soeur`,
        `продолжала Анна Павловна после минутного молчания, придвигаясь к князю и ласково`,
        `И он с теми свободными и фамильярными грациозными движениями, которые его отличали, взял за руку фрейлину, поцеловал ее и, поцеловав, помахал фрейлинскою рукой, развалившись на креслах и глядя в сторону`,
    ].forEach(pattern => {

        [16, 32, 64, 128 /* ISO8859-1 */, 256 /* ASCII */, 1024, 8192, 23770  /* GBK */, 128237 /* UTF-8 */].forEach(length => {
            const suite = new Suite(`BM Array Len ${length} vs Map | PLen ${pattern.length}`);

            console.log(`\n\nBM Array Len ${length} vs Map | PLen ${pattern.length}\n`)

            suite
                .add('getSubstringBMBadCharacterMap', () => getSubstringBMBadCharacterMap(text, pattern))
                .add('getSubstringBMBadCharacterArray', () => getSubstringBMBadCharacterArray(text, pattern, length))
